...
class ComposeImageRTTTestAPITestCase(TestCaseWithChangeSetMixin, APITestCase):
    ...
    def test_query_location(self):
        response = self.client.get(reverse('composeimagertttests-list'), {"location": "NAY"})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['count'], 1)

    def test_query_location_nonexisting(self):
        response = self.client.get(reverse('composeimagertttests-list'), {"location": "does-not-exist"})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['count'], 0)

    def test_query_scheme(self):
        response = self.client.get(reverse('composeimagertttests-list'), {"scheme": "nfs"})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['count'], 2)

    def test_query_scheme_nonexisting(self):
        response = self.client.get(reverse('composeimagertttests-list'), {"scheme": "does-not-exist"})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['count'], 0)
    ...
    def test_delete_existing(self):
        response = self.client.delete(reverse('composeimagertttests-detail',
                                              args=['compose-1/Server/x86_64/NAY']))
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        self.assertEqual(models.ComposeTree.objects.count(), 1)
        self.assertNumChanges([1])
    ...
    def test_bulk_delete(self):
        data = ['compose-1/Server/x86_64/NAY', 'compose-1/Server2/x86_64/BRQ']
        response = self.client.delete(reverse('composeimagertttests-list'), data, format='json')
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        self.assertEqual(models.ComposeTree.objects.count(), 0)
        self.assertNumChanges([2])